 ...
 */
class CompositeDef extends ComponentDef implements Codec<Map<Integer, Object>> {

    Codec<Bitmap> bitmapCodec;

    SortedMap<Integer, ComponentDef> componentDefs = new TreeMap<>();

    CompositeCodec compositeCodec;

    CompositeDef() {
        super.dataCodec = this;
    }

    @Override
    public Map<Integer, Object> decode(ByteBuffer buf) {
        return compositeCodec.decode(buf, componentDefs);
    }

    @Override
    public void encode(ByteBuffer buf, Map<Integer, Object> value) {
        compositeCodec.encode(buf, value, componentDefs);
    }
